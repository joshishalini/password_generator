module Password
  ...
  class Validator
    ...
    def check_presence
      %i[length uppercase lowercase number special].each do |param|
        if instance_variable_get("@#{param}").nil?
          errors << "#{param.capitalize} must be provided."
        end
      end
    end
    ...
    def check_letter_flags
			unless [true, false].include?(uppercase)
        errors << "Uppercase must be a boolean value."
      end

      unless [true, false].include?(lowercase)
        errors << "Lowercase must be a boolean value."
      end

      if [true, false].include?(uppercase) && [true, false].include?(lowercase) && !uppercase && !lowercase
